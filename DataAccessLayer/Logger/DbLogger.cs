...
namespace DataAccessLayer.Logger
{
    public class DbLogger : ILogger
    {
        ...
        public DbLogger([NotNull] DbLoggerProvider dbLoggerProvider, DapperContext context)
        {
        ...
        public void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception exception, Func<TState, Exception, string> formatter)
        {
            ...
            #region stored using sql connection
            //Store record.
            ...
            var query = string.Format("INSERT INTO {0} ([Values], [Created]) VALUES (@Values, @Created)", _dbLoggerProvider.Options.LogTable);
            var parameters = new DynamicParameters();
            parameters.Add("Values", JsonConvert.SerializeObject(values, new JsonSerializerSettings
            {
                NullValueHandling = NullValueHandling.Ignore,
                DefaultValueHandling = DefaultValueHandling.Ignore,
                Formatting = Formatting.None
            }).ToString());
            parameters.Add("Created", TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, TimeZoneInfo.FindSystemTimeZoneById("India Standard Time")), DbType.DateTime);
