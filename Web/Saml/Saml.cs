...
namespace OneLogin
{
    namespace Saml
    {
        ...
        public class Response
        {
            ...
            public string GetSAMLAppName()
            {
                ...
                if (nodes.Count > 0)
                {
                    for (int i = 0; i < nodes.Count; i++)
                    {
                        if (nodes[i].Attributes[0].InnerText.Trim() == "app_name")
                        {
                            role = nodes[i].InnerText.Trim();
                            break;
                        }
        ...
        public class AuthRequest
        {
            ...
            public string GetLogOutRequest(AuthRequestFormat format, string issuer, string logouturl, string entityid)
            {
                using (StringWriter sw = new StringWriter())
                {
                    XmlWriterSettings xws = new XmlWriterSettings();
                    ...
                    using (XmlWriter xw = XmlWriter.Create(sw, xws))
                    {
                        ...
                        xw.WriteAttributeString("ID", entityid);

                        xw.WriteAttributeString("Version", "2.0");
                        xw.WriteAttributeString("IssueInstant", DateTime.Now.ToString());
                        xw.WriteAttributeString("Destination", logouturl);
                        xw.WriteAttributeString("InResponseTo", "emIDAMEntityId01");
                        xw.WriteAttributeString("Issuer", entityid);
            ...
            public static string EncryptData(string logoutRequest)
            {
                ...
                using (var aesAlg = new AesManaged() { KeySize = 256 })
                {
                    ...
                    using (var encryptor = aesAlg.CreateEncryptor())
                    {
                        ...
                        var certPath = Environment.GetEnvironmentVariable("Class3__path") ?? string.Empty;

                        var publicKey = new X509Certificate2("C:\\Cert\\IAM Public Key\\class3.cer");
