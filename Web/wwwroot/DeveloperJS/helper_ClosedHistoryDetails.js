                ...
                if (ICardHistory?.length > 0) {
                    ...
                    listItem = ICardHistory.map((item, index) => {
                        ...
                        const postingOutBeforeHtml = postingOutBeforeForward
                            .map(posting => createPostingOutHtml_ClosedHistory(posting))
                            .join("");
...
function createPostingOutHtml_ClosedHistory(posting) {
    return `
...
function SetClosedHistoryHeader(requestId) {
    ...
    $("#exampleModalLabel_BasicDetailClosedHistory").html(header);
}

function GenerateClosedHistoryPDF(RequestId) {
    try {
