...
function DeleteSignature(Id) {
    if (Id == '' || Id == null || typeof Id == "undefined") {
        alert("Signature not saved in DB.");
        return false;
    }
    $.ajax({
        url: "/BasicDetail/DeleteSignature",
        type: "POST",
        data: {
            "Id": Id
        },
        success: function (response, status) {
            if (response.success) {
                $("#btnsignaturedelete").prop('disabled', true);
...
function DeletePhoto(Id) {
    if (Id == '' || Id == null || typeof Id == "undefined") {
        alert("Photo not saved in DB.");
        return false;
    }
    $.ajax({
        url: "/BasicDetail/DeletePhoto",
        type: "POST",
        data: {
            "Id": Id
        },
        success: function (response, status) {
            if (response.success) {
                $("#btnphotodelete").prop('disabled', true);
                alert(response.message);
                $("#PhotoPath").attr({
                    'src': '/writereaddata/images/noimage.png',
                    'width': '200px',
...
function beforeSaveValidateBasicDetail(id) {
    let formId = '#' + id;
    ...
    $('#Signature_-error').html('');
    $('#Signature_').prop('required', false);
    $('#Photo_-error').html('');
    $('#Photo_').prop('required', false);
    $('#PlaceOfIssue').attr('required', false);
    $('#ServiceNo').prop('required', true);
    ...
    if ($(formId).valid()) {
        ...
        var Signature_Input = $('#Signature_')[0];
        var Signaturefile = Signature_Input.files[0];
        fdata.append("Signature_", Signaturefile);

        var Photo_Input = $('#Photo_')[0];
        var Photofile = Photo_Input.files[0];
        fdata.append("Photo_", Photofile);

        fdata.append("BasicDetailId", $("#BasicDetailId").val());
        fdata.append("Name", $("#Name").val());
        fdata.append("Rank", $("#Rank").val());
        fdata.append("ArmService", $("#ArmService").val());
        fdata.append("ServiceNo", $("#ServiceNo").val());
        fdata.append("IdentityMark", $("#IdentityMark").val());
        fdata.append("DOB", $("#DOB").val());
        fdata.append("Height", $("#Height").val());
        fdata.append("AadhaarNo", $("#AadhaarNo").val());
        fdata.append("BloodGroup", $("#BloodGroup").val());
        fdata.append("PlaceOfIssue", $("#PlaceOfIssue").val());
        fdata.append("DistrictId", $("#DistrictId").val());
        fdata.append("PlaceOfIssue", $("#PlaceOfIssue").val());
        fdata.append("DateOfIssue", $("#DateOfIssue").val());
        fdata.append("IssuingAuth", $("#IssuingAuth").val());
        fdata.append("DateOfCommissioning", $("#DateOfCommissioning").val());
        fdata.append("PermanentAddress", $("#PermanentAddress").val());

        $.ajax({
            url: "/BasicDetail/SaveBasicDetailWithSerialize",
            type: "POST",
            data: fdata,
            processData: false,
            contentType: false,
            success: function (response) {
                ...
                if (response.PhotoImagePath != null) {
                    $("#PhotoPath").val(response.PhotoImagePath);
                    $("#btnphotodelete").prop('disabled', false);
                }
                else {
                    $("#btnphotodelete").prop('disabled', true);
                }
                if (response.SignatureImagePath != null) {
                    $("#SignaturePath").val(response.SignatureImagePath);
                    $("#btnsignaturedelete").prop('disabled', false);
                }
                else {
                    $("#btnsignaturedelete").prop('disabled', true);
                }

                $("#EncryptedId").val(response.EncryptedId);
                $("#BasicDetailId").val(response.BasicDetailId);
                $("#Name").val(response.Name);
                $("#Rank").val(response.Rank);
                $("#ArmService").val(response.ArmService);
                $("#ServiceNo").val(response.ServiceNo);
                $("#initialServiceNo").val(response.ServiceNo);
