...
/*dragula([$('Posted-offres'), $('TD-Offrs')]);*/
    dragula(
        [document.querySelector('#Posted-offres')].concat(
            Array.from(document.querySelectorAll('.my-container'))
        ), {
        revertOnSpill: true
    });
//}).on('drop', function (el) {
//    if ($('Leave-Offrs').children.length > 0) {
//        //$('lvedisplay').innerHTML = $('TD-Leave').innerHTML;

//        //$('#TD-Leave img').each(function () {
//        //    alert($(this).attr('src'))
//        //});
//        //alert(el.parentElement.id)
//        if (jQuery.inArray(el.id, lve) == -1) {
//            lve.push(el.id);
//            //$('lvedisplay').innerHTML = el.id;
//            sessionStorage.setItem("Leave", el.id);
//        } else {
//            lve.pop(el.id);
//        }
...
//        lve.pop(el.id);
//    }
//    if ($('TD-Offrs').children.length > 0) {
//        sessionStorage.setItem("TD", el.id);
//    }
